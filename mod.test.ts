import { url, unsafeComponent, urlRelativeTo } from "./mod.ts";
...
Deno.test("URL with an accidental query string is escaped", () => {
  const x = url`http://example.com/first/${'?unexpected='}/third`;
  assertEquals(x.toString(), 'http://example.com/first/%3Funexpected%3D/third');
});
...
Deno.test("URLs can be created relative to another", () => {
